    ...
    const results = data.list.map((item: ApiSearchItem) => {
      ...
      if (item.vod_play_url) {
        ...
        vod_play_url_array.forEach((url: string) => {
          ...
          if (matches.length > episodes.length) episodes = matches;
        });
      }
...
export async function getDetailFromApi(
  ...
  const hasResolution = RESOLUTION_REGEX.test(videoDetail.vod_name);
  if (!hasResolution) {
    console.log(`[过滤] 详情页源无清晰度标识: ${videoDetail.vod_name}`);
    return {
      id: id.toString(),
...
async function handleSpecialSourceDetail(
  ...
  // 关键：特殊源也做清晰度过滤
  const hasResolution = RESOLUTION_REGEX.test(titleText);
  if (!hasResolution) {
    console.log(`[过滤] 特殊源详情无清晰度标识: ${titleText}`);
    return {
      id,
      title: titleText,
      poster: coverUrl,
