...
echo "🚀 [Arch] 正在应用 V53 基础设施固化版 (移除测速，回归纯净透传模式)..."

# -----------------------------------------------------------------------------
# 1. 🏗️ 基础设施固化 (免密、权限、UI 屏蔽)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 2. 🚦 纯净透传 API 实现 (移除所有测速代码)
# -----------------------------------------------------------------------------

# 重写Search API - 仅搜索，不测速
...
# 移除不需要的页面和API
rm -rf src/app/login src/app/warning src/app/api/change-password src/lib/smartTest.ts

echo "✅ [Arch] V53 补丁应用成功！系统纯净透传模式。"
EOF
